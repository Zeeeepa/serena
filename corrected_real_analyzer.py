#!/usr/bin/env python3
"""
Corrected Real Error Analyzer - Fixed builtin detection and scope analysis

This analyzer properly handles Python builtins, imports, and scope resolution
to show only actual errors that would occur at runtime.
"""
...
class CorrectedScopeAnalyzer(ast.NodeVisitor):
    """Corrected analyzer with proper builtin and scope handling."""
    ...
    def __init__(self, file_path: str):
        ...
        # Get actual Python builtins correctly
        self.builtin_names = set(dir(builtins))
        
        # Add commonly available standard library modules
        self.builtin_names.update({
            'os', 'sys', 'json', 'math', 'random', 'datetime', 'time', 
            'collections', 'itertools', 'functools', 'operator', 're', 
            'string', 'pathlib', 'subprocess', 'tempfile', 'shutil',
            'argparse', 'logging', 'importlib', 'ast', 'typing'
        })
        
        # Add special names that are always available
        self.builtin_names.update({
            '__name__', '__file__', '__doc__', '__package__', '__spec__',
            '__builtins__', '__cached__', '__loader__'
        })
    ...
    def add_error(self, node: ast.AST, error_type: str, message: str, 
                  severity: str = "ERROR", category: str = "real_error", 
                  fix_suggestion: Optional[str] = None):
    ...
    def check_import_availability(self, module_name: str) -> bool:
        """Check if a module can be imported."""
        if module_name in self.failed_imports:
            return False
        
        # Don't check relative imports or special cases
    ...
    def visit_Import(self, node: ast.Import):
        """Handle import statements with proper availability checking."""
        for alias in node.names:
            module_name = alias.name
            import_name = alias.asname if alias.asname else alias.name.split('.')[0]
            
            # Check if module is available (only for non-standard modules)
            if module_name not in self.builtin_names and not self.check_import_availability(module_name):
                # Only report as error if it's clearly a third-party module
                if not module_name.startswith('_') and '.' not in module_name:
                    self.add_error(
                        ...
                        "ERROR",
                        "import_error",
                        f"Install the module with: pip install {module_name}"
                    )
            else:
                self.imports.add(import_name)
                self.add_name(import_name)
        
        self.generic_visit(node)
    
    def visit_ImportFrom(self, node: ast.ImportFrom):
        """Handle from ... import statements with proper availability checking."""
        if node.module:
            # Check if the base module is available (only for non-standard modules)
            if (node.module not in self.builtin_names and 
                not node.module.startswith('.') and 
                not self.check_import_availability(node.module)):
                # Only report as error if it's clearly a third-party module
                if not node.module.startswith('_') and 'test' not in node.module.lower():
                    self.add_error(
                        node,
                        "ImportError",
                        f"No module named '{node.module}'",
                        "ERROR",
                        "import_error",
                        f"Install the module with: pip install {node.module.split('.')[0]}"
                    )
                    return
        
        for alias in node.names:
            if alias.name == '*':
                # Can't easily resolve * imports, so skip detailed checking
                continue
    ...
    def visit_Name(self, node: ast.Name):
        """Handle name references with proper builtin checking."""
        if isinstance(node.ctx, ast.Load):
            # This is a name being used (not assigned)
            if not self.is_name_defined(node.id):
                # Provide helpful fix suggestions
                fix_suggestion = None
                if node.id in ['os', 'sys', 'json', 'math', 'random', 'datetime']:
                    fix_suggestion = f"Add 'import {node.id}' at the top of the file"
                elif node.id.endswith('Error') or node.id.endswith('Exception'):
                    fix_suggestion = f"'{node.id}' might be a built-in exception, check spelling or import"
                else:
                    fix_suggestion = f"Define '{node.id}' before using it, or check for typos"
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name):
            if not self.is_name_defined(node.func.id):
                fix_suggestion = f"Define function '{node.func.id}' or import it from a module"
                self.add_error(
                    node,
                    "NameError",
...
class CorrectedRealAnalyzer:
    """Corrected analyzer that finds only real errors."""
    ...
    def clone_repository(self, repo_url: str) -> str:
        ...
        temp_dir = tempfile.mkdtemp(prefix="corrected_analysis_")
        ...
        try:
            ...
            try:
                ...
                return [RealError(
                    ...
                    fix_suggestion="Fix the syntax error according to Python grammar rules"
                )]
            
            # Analyze for real errors
            analyzer = CorrectedScopeAnalyzer(file_path)
            analyzer.visit(tree)
            ...
            return [RealError(
                ...
                fix_suggestion="Save the file with UTF-8 encoding"
            )]
        except Exception as e:
            if self.verbose:
    ...
    def format_output(self, results: Dict[str, Any], output_format: str = "text") -> str:
        ...
        if not errors:
            return "ERRORS: ['0']\nNo real errors found. ✅ All code looks good!"
        ...
        for i, error in enumerate(sorted_errors, 1):
            ...
            line = f"{i}. '{location}' '{file_name}' '{error.message}' 'severity: {error.severity}, type: {error.error_type}'"
            lines.append(line)
        ...
        lines.append("")
        lines.append("=" * 70)
        lines.append("🎯 CORRECTED REAL ERROR ANALYSIS SUMMARY")
        lines.append("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Corrected Real Error Analyzer - Only shows actual Python errors",
...
This corrected analyzer uses proper builtin detection and scope resolution 
to show only real errors that would actually occur when running Python code.
        """
    )
    ...
    if args.verbose:
        print("🚀 CORRECTED REAL ERROR ANALYZER")
        print("=" * 70)
    ...
    # Run analysis
    analyzer = CorrectedRealAnalyzer(verbose=args.verbose)

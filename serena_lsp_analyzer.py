...
class SerenaLSPAnalyzer:
    ...
    def __init__(
        self, verbose: bool = False, timeout: float = 600, max_workers: int = 4
    ):
        """
        ...
        if verbose:
            ...
            self.logger.info(
                f"⚙️  Configuration: timeout={timeout}s, max_workers={max_workers}"
            )
        ...
        try:
            ...
            if self.language_server:
                try:
                    if self.language_server.is_running():
                        ...
                        if self.language_server.is_running():
                            self.logger.warning(
                                "⚠️  Language server did not stop gracefully"
                            )
                    ...
                    self.logger.warning(
                        f"⚠️  LSP Error during server shutdown: {lsp_error}"
                    )
                    ...
                    lsp_error = LSPError(
                        ErrorCodes.InternalError, f"Error stopping server: {str(e)}"
                    )
                    self.server_info.last_error = lsp_error
                    self.server_info.error_count += 1
            ...
            if self.temp_dir and os.path.exists(self.temp_dir):
                try:
                    ...
                    self.logger.warning(
                        f"⚠️  Error cleaning up temporary directory: {e}"
                    )
            ...
            if self.verbose and self.server_info.message_count > 0:
                ...
                self.logger.info(
                    f"   📨 Total messages: {self.server_info.message_count}"
                )
                self.logger.info(f"   ❌ Total errors: {self.server_info.error_count}")
                self.logger.info(
                    f"   ⏱️  Initialization time: {self.server_info.initialization_time:.2f}s"
                )
                self.logger.info(
                    f"   📊 Final status: {self.server_info.server_status}"
                )
        ...
        for root, dirs, files in os.walk(repo_path):
            ...
            for file in files:
                ...
                for lang, indicators in language_indicators.items():
                    if any(
                        file_lower.endswith(ext) or file_lower == indicator
                        for ext in indicators
                        for indicator in [ext]
                        if ext.startswith(".")
                    ):
                        file_counts[lang] += 1
                    elif any(
                        file_lower == indicator
    ...
    def setup_project(
        self, repo_path: str, language: Optional[Language] = None
    ) -> Project:
        """
        ...
        self.logger.info(
            f"⚙️  Setting up project for {repo_path} with language {language.value}"
        )
    ...
    def start_language_server(self, project: Project) -> SolidLanguageServer:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                self.logger.info(
                    f"🚀 Language server initialization attempt {attempt + 1}/{max_attempts}"
                )
                ...
                try:
                    ...
                    if not self.language_server:
                        raise LSPError(
                            ...
                            "Failed to create language server instance",
                        )

                    # Store process information if available
                    if hasattr(self.language_server, "process_info"):
                        self.server_info.process_info = (
                            self.language_server.process_info
                        )
                    ...
                    if not isinstance(e, LSPError):
                        lsp_error = LSPError(
                            ...
                            f"Server creation failed: {str(e)}",
                        )
                    else:
                        lsp_error = e
                    self.server_info.last_error = lsp_error
                ...
                try:
                    if not self.language_server.is_running():
                        raise LSPError(
                            ErrorCodes.ServerNotInitialized,
                            "Language server process is not running after start",
                        )
                    ...
                    for i in range(int(ready_timeout)):
                        if not self.language_server.is_running():
                            raise LSPError(
                                ErrorCodes.ServerNotInitialized,
                                f"Server stopped during initialization at {i}s",
                            )

                        if self.verbose and i % 5 == 0:
                            self.logger.debug(
                                f"🔍 Server readiness check: {i}/{ready_timeout}s"
                            )
                    ...
                    if not self.language_server.is_running():
                        raise LSPError(
                            ErrorCodes.ServerNotInitialized,
                            "Language server stopped running during initialization",
                        )
                ...
                if self.verbose and self.server_info.process_info:
                    self.logger.info(
                        f"📊 Process info: cmd={self.server_info.process_info.cmd}, cwd={self.server_info.process_info.cwd}"
                    )

                self.logger.info(
                    f"🎉 Language server successfully initialized in {total_init_time:.2f}s"
                )
                self.logger.info(
                    f"📈 Server stats: messages={self.server_info.message_count}, errors={self.server_info.error_count}"
                )
                ...
                if attempt < max_attempts - 1:
                    ...
                    raise RuntimeError(
                        f"Failed to start language server after {max_attempts} attempts: {lsp_error}"
                    )

            except Exception as e:
                # Convert unexpected errors to LSPError
                lsp_error = LSPError(
                    ErrorCodes.InternalError, f"Unexpected error: {str(e)}"
                )
                self.server_info.last_error = lsp_error
                self.server_info.error_count += 1
                ...
                if attempt < max_attempts - 1:
                    ...
                    self.server_info.server_status = "failed"
                    raise RuntimeError(
                        f"Failed to start language server after {max_attempts} attempts: {e}"
                    )
    ...
    def get_server_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_error": str(self.server_info.last_error)
            if self.server_info.last_error
            else None,
            "process_info": {
                "cmd": str(self.server_info.process_info.cmd)
                if self.server_info.process_info
                else None,
                "cwd": self.server_info.process_info.cwd
                if self.server_info.process_info
                else None,
                "env_vars": len(self.server_info.process_info.env)
                if self.server_info.process_info
                else 0,
            }
            if self.server_info.process_info
            else None,
            "is_running": self.language_server.is_running()
            if self.language_server
            else False,
        }
    ...
    def collect_diagnostics(
        ...
        self.logger.info(
            f"🚀 Processing ALL {self.total_files} files with Serena LSP analysis..."
        )
        self.logger.info(f"⚙️  Using {self.max_workers} workers with batch processing")

        def analyze_single_file(
            file_path: str, retry_count: int = 0
        ) -> Tuple[str, List[EnhancedDiagnostic], Optional[str], bool]:
            ...
            try:
                ...
                try:
                    lsp_diagnostics = language_server.request_text_document_diagnostics(
                        file_path
                    )
                    ...
                    if lsp_error.code == ErrorCodes.RequestCancelled:
                        if self.verbose:
                            self.logger.debug(
                                f"🔄 Request cancelled for {os.path.basename(file_path)}: {lsp_error.message}"
                            )
                        # Retryable error
                        return (
                            file_path,
                            [],
                            f"LSP Request Cancelled: {lsp_error.message}",
                            retry_count < 2,
                        )
                    elif lsp_error.code == ErrorCodes.ContentModified:
                        if self.verbose:
                            self.logger.debug(
                                f"📝 Content modified for {os.path.basename(file_path)}: {lsp_error.message}"
                            )
                        # Retryable error
                        return (
                            file_path,
                            [],
                            f"LSP Content Modified: {lsp_error.message}",
                            retry_count < 2,
                        )
                    elif lsp_error.code == ErrorCodes.ServerNotInitialized:
                        self.logger.error(
                            f"❌ Server not initialized for {os.path.basename(file_path)}: {lsp_error.message}"
                        )
                        # Non-retryable error
                        return (
                            file_path,
                            [],
                            f"LSP Server Not Initialized: {lsp_error.message}",
                            False,
                        )
                    else:
                        # Log with appropriate message type based on error severity
                        if lsp_error.code in [
                            ErrorCodes.InternalError,
                            ErrorCodes.ServerErrorStart,
                        ]:
                            self.logger.error(
                                f"❌ LSP Internal Error for {os.path.basename(file_path)}: {lsp_error}"
                            )
                        else:
                            self.logger.warning(
                                f"⚠️  LSP Error for {os.path.basename(file_path)}: {lsp_error}"
                            )
                        ...
                        return (
                            file_path,
                            [],
                            f"LSP Error ({lsp_error.code}): {lsp_error.message}",
                            retryable,
                        )
                ...
                for diag in lsp_diagnostics:
                    ...
                    column = (
                        start_pos.get("character", 0) + 1
                    )  # LSP uses 0-based character numbers
                ...
                if self.verbose and len(enhanced_diagnostics) > 0:
                    if any(d.severity == "ERROR" for d in enhanced_diagnostics):
                        ...
                        self.logger.debug(
                            f"🔴 Found {len(enhanced_diagnostics)} diagnostics (including errors) in {os.path.basename(file_path)}"
                        )
                    elif any(d.severity == "WARNING" for d in enhanced_diagnostics):
                        # Use warning message type for files with warnings
                        self.logger.debug(
                            f"🟡 Found {len(enhanced_diagnostics)} diagnostics (warnings) in {os.path.basename(file_path)}"
                        )
                    else:
                        # Use info message type for files with info/hints
                        self.logger.debug(
                            f"🔵 Found {len(enhanced_diagnostics)} diagnostics (info/hints) in {os.path.basename(file_path)}"
                        )
                ...
                if not isinstance(e, LSPError):
                    lsp_error = LSPError(
                        ErrorCodes.InternalError,
                        f"Unexpected error analyzing {file_path}: {str(e)}",
                    )
                    self.server_info.last_error = lsp_error

                # Log with error message type
                self.logger.warning(
                    f"⚠️  Unexpected error analyzing {os.path.basename(file_path)}: {e}"
                )

                # Determine if this is a retryable error
                retryable = retry_count < 2 and any(
        ...
        self.logger.info(
            f"📦 Processing {len(file_batches)} batches of ~{batch_size} files each"
        )
        ...
        for batch_idx, file_batch in enumerate(file_batches):
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        analyzed_file, diagnostics, error, should_retry = (
                            future.result()
                        )
                        ...
                        with self.lock:
                            if error is None:
                                ...
                                if self.verbose and len(diagnostics) > 0:
                                    self.logger.debug(
                                        f"✅ Found {len(diagnostics)} diagnostics in {os.path.basename(analyzed_file)}"
                                    )
                            elif should_retry:
                                retry_files.append(analyzed_file)
                                if self.verbose:
                                    self.logger.debug(
                                        f"🔄 Queued for retry: {os.path.basename(analyzed_file)}: {error}"
                                    )
                            else:
                                self.failed_files += 1
                                self.logger.warning(
                                    f"⚠️  Failed to analyze {os.path.basename(analyzed_file)}: {error}"
                                )
                            ...
                            if (
                                current_progress - last_progress_report
                                >= progress_interval
                            ):
                                ...
                                eta = (
                                    (self.total_files - current_progress) / rate
                                    if rate > 0
                                    else 0
                                )

                                self.logger.info(
                                    f"📈 Progress: {current_progress}/{self.total_files} ({percentage:.1f}%) "
        ...
        if retry_files:
            self.logger.info(
                f"🔄 Retrying {len(retry_files)} files that had transient failures..."
            )
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        if error is None:
                            ...
                            if self.verbose:
                                self.logger.debug(
                                    f"✅ Retry successful: {len(diagnostics)} diagnostics in {os.path.basename(analyzed_file)}"
                                )
                        else:
                            self.failed_files += 1
                            self.logger.warning(
                                f"⚠️  Retry failed for {os.path.basename(analyzed_file)}: {error}"
                            )

                    except Exception as e:
                        self.failed_files += 1
                        self.logger.error(
                            f"❌ Retry error for {os.path.basename(file_path)}: {e}"
                        )
        ...
        self.logger.info(
            f"📊 Total files analyzed: {self.processed_files + self.failed_files}/{self.total_files}"
        )
        ...
        self.logger.info(
            f"🚀 Processing rate: {(self.processed_files + self.failed_files) / analysis_time:.2f} files/sec"
        )
        self.logger.info("=" * 80)
    ...
    def format_diagnostic_output(self, diagnostics: List[EnhancedDiagnostic]) -> str:
        ...
        for i, diag in enumerate(diagnostics, 1):
            ...
            diagnostic_line = (
                f"{i}. '{location}' '{file_name}' '{clean_message}' '{other_types}'"
            )
        ...
        if self.verbose or error_count > 50:
            ...
            for diag in diagnostics:
                severity_counts[diag.severity] = (
                    severity_counts.get(diag.severity, 0) + 1
                )
    ...
    def analyze_repository(
        ...
        try:
            ...
            self.logger.info(
                f"🌐 Language override: {language_override or 'AUTO-DETECT'}"
            )
            ...
            if language_override:
                try:
                    ...
                    self.logger.warning(
                        f"⚠️  Invalid language '{language_override}', will auto-detect"
                    )
            ...
            if severity_filter:
                ...
                if severity is None:
                    self.logger.warning(
                        f"⚠️  Invalid severity '{severity_filter}', showing all diagnostics"
                    )
                else:
                    self.logger.info(
                        f"🔍 Filtering by severity: {severity_filter.upper()}"
                    )
            ...
            self.logger.info(
                f"⏳ Allowing {initialization_time}s for LSP initialization..."
            )
            ...
            self.logger.info(
                "🔍 Beginning comprehensive Serena LSP diagnostic collection..."
            )
            ...
            if output_format == "json":
                results = self._generate_results(
                    diagnostics, project.language.value, repo_path, total_start_time
                )
                ...
                self.logger.info(
                    f"   📥 Repository setup: {self.performance_stats['clone_time']:.2f}s"
                )
                self.logger.info(
                    f"   ⚙️  Project configuration: {self.performance_stats['setup_time']:.2f}s"
                )
                self.logger.info(
                    f"   🔧 LSP server startup: {self.performance_stats['lsp_start_time']:.2f}s"
                )
                self.logger.info(
                    f"   🔍 Diagnostic analysis: {self.performance_stats['analysis_time']:.2f}s"
                )
                self.logger.info(f"   🎯 Total execution time: {total_time:.2f}s")

                if self.total_files > 0:
                    self.logger.info(
                        f"   📊 Average time per file: {self.performance_stats['analysis_time'] / self.total_files:.3f}s"
                    )
...
🔧 SUPPORTED LANGUAGES:
  Python, TypeScript/JavaScript, Java, C#, C++, Rust, Go, PHP, Ruby, Kotlin, Dart
    ...
    if args.timeout < 60:
        print(
            "⚠️  Warning: Timeout less than 60 seconds may cause issues with large repositories"
        )
    ...
    try:
        with SerenaLSPAnalyzer(
            verbose=args.verbose, timeout=args.timeout, max_workers=args.max_workers
        ) as analyzer:
